...
class AgentCoreSDKManifest:
    ...
    contract_api: tuple[str, ...] = STABLE_PUBLIC_API
    ...
    def manifest(self) -> dict[str, Any]:
        ...
        return {
            ...
            "contract_api_count": len(self.contract_api),
            "contract_api": list(self.contract_api),
            "root_export_count": len(self.root_exports),
            "root_export_sample": list(self.root_exports[:25]),
            "api_contract": agent_core_api_contract(self.contract_api).manifest(),
...
class AgentCoreReadinessProfile:
    ...
    def evaluate(self, sdk_manifest: AgentCoreSDKManifest | dict[str, Any]) -> AgentCoreReadinessReport:
        ...
        public_api = _manifest_api_surface(manifest)
...
def default_agent_core_capabilities() -> tuple[AgentCoreCapability, ...]:
    ...
    return (
        ...
        AgentCoreCapability(
            ...
            runtime_notes=(
                "Concrete HTTP clients, vendor SDKs, credentials, endpoint routing, and tenant policy stay outside core.",
            ),
        ),
        AgentCoreCapability(
            name="task_contract_profile",
            layer="harness",
            status="stable_contract",
            summary="Portable run-input contract profile for generic, context-aware, memory-backed, managed, and tool/structured tasks.",
            public_contracts=(
                "AgentTaskContract",
                "AgentCoreTaskContractProfile",
                "agent_core_task_contract_profile",
            ...
            runtime_notes=(
                "Domain prompts, tenant identity, product workflow metadata, worker scheduling, and concrete tools stay outside core.",
            ),
        ),
        AgentCoreCapability(
        ...
        AgentCoreCapability(
            name="capability_governance_acceptance_harness",
            layer="eval",
            summary="Pure-SDK disabled capability visibility, route-plan audit, MCP failure isolation, and skill invocation governance checks.",
            public_contracts=(
                "AgentCoreCapabilityGovernanceAcceptanceHarness",
                "AgentCoreCapabilityGovernanceAcceptanceReport",
            ),
            runtime_notes=(
                "Operator identity, product permission policy, distributed MCP supervision, and approval UX stay outside core.",
            ),
        ),
        AgentCoreCapability(
        ...
        AgentCoreCapability(
            name="concurrency_acceptance_harness",
            layer="eval",
            summary="Pure-SDK concurrent session isolation checks for manager runs, provider calls, tools, events, traces, and eval.",
            public_contracts=(
                "AgentCoreConcurrencyAcceptanceHarness",
                "AgentCoreConcurrencyAcceptanceReport",
            ),
            runtime_notes=(
                "Runtime-specific worker pools, distributed locks, process supervision, and UI scheduling stay outside core.",
            ),
        ),
        AgentCoreCapability(
        ...
        AgentCoreCapability(
            name="interrupt_acceptance_harness",
            layer="eval",
            summary="Pure-SDK pre-cancel, provider/tool timeout, manager cancel reset, event, trace, and eval checks.",
            public_contracts=(
                "AgentCoreInterruptAcceptanceHarness",
                "AgentCoreInterruptAcceptanceReport",
            ),
            runtime_notes=(
                "Operator controls, distributed process cancellation, and UI interruption workflows stay outside core.",
            ),
        ),
        AgentCoreCapability(
...
def agent_core_replacement_readiness_profile() -> AgentCoreReadinessProfile:
    ...
    return AgentCoreReadinessProfile(
        ...
        required_capabilities=(
            ...
            "concurrency_acceptance_harness",
        ...
        required_public_api=STABLE_PUBLIC_API,
...
def evaluate_agent_core_api_lifecycle(
    ...
    *,
    ...
    public_api = _manifest_api_surface(manifest, include_root_exports=False)
...
def agent_core_sdk_manifest(
    *,
    public_api: Sequence[str] = (),
    contract_api: Sequence[str] = STABLE_PUBLIC_API,
    root_exports: Sequence[str] = (),
    ...
    return AgentCoreSDKManifest(
        ...
        contract_api=tuple(sorted(set(contract_api))),
        root_exports=tuple(sorted(set(root_exports))),
        metadata=dict(metadata or {}),
    )
...
def _manifest_api_surface(
    manifest: dict[str, Any],
    *,
    include_root_exports: bool = True,
) -> set[str]:
    names = {str(name) for name in manifest.get("public_api", ())}
    names.update(str(name) for name in manifest.get("contract_api", ()))
    if include_root_exports:
        names.update(str(name) for name in manifest.get("root_exports", ()))
    return names
